...
T6 fast path: instead of running R_DrawSurfaceBlock8_mipN to bake
lightmap × texture × colormap into a 2 MB CPU surface cache, point
the GPU directly at the raw mip texture and submit spans with
OF_EMIT_COLORMAP + a per-surface light byte. The GPU then does
`colormap[light * 256 + texel]` per pixel in hardware.

The CPU still runs R_BuildLightMap to know what light to send, but
that's ~324 entries per surface vs the ~thousands of texels the block
drawer would touch. Big net win when the surface cache would have
missed; small loss vs a hot cache hit (we always rebuild lightmap
here, no surface-level cache).

Sets cacheblock + cachewidth and returns a colormap row index in [0,63]
for the span loop to embed in OF_EMIT_COLORMAP spans.

Loses intra-surface lightmap variation — every pixel of the surface
gets the same row 0 lightmap value. That's the "simple variant" T6
tradeoff. Fix later by tessellating world surfaces into smaller
triangles that ride the GPU's per-vertex Gouraud light path, once the
gateware perspective + Gouraud-colormap chain is verified on hw.
================
*/
PQ_HOT byte D_GpuLightSurface (msurface_t *surface, int miplevel)
{
	texture_t *mt = R_TextureAnimation (surface->texinfo->texture);

	r_drawsurf.texture     = mt;
	 ...
	 * to colormap rows [0..63] same as `(llight & 0xFF00) >> 8` in
	 * the original CPU walk. Pick the top-left lightmap entry as a
	 * representative — for surfaces with uniform lighting this is
	 * exact; for those with strong shadow gradients it's the simple-
	 * variant tradeoff. */
	int lt = (int)(blocklights[0] >> 8);
	if (lt < 0)  lt = 0;
	if (lt > 63) lt = 63;
	return (byte)lt;
}
