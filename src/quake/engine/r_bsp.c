...
/* =====================================================================
 * World-surface SoA cache (cache-locality optimization).
 *
 * Quake's R_RecursiveWorldNode mark loop writes `(*mark)->visframe` for
 * every surface in each visible leaf, and its descent loop reads
 * `surf->visframe` and `surf->flags` before calling R_RenderFace.
 *
 * msurface_t is 64 bytes, so every one of those writes/reads forces a
 * full cache-line fill. On a moderately crowded level (5-10 k
 * surfaces) that's 320-640 KB of line traffic — far over the 32 KB
 * D-cache budget on this platform.
 *
 * Splitting the two hottest fields into parallel dense SoAs drops the
 * per-surface working set to 5 bytes (4 B visframe + 1 B flags), so a
 * cache line now covers ~12 surfaces and leaves room for other hot
 * state (surfaces[] array itself, mplane_t, etc.). The mark loop's
 * write becomes a sequential store into a packed uint32_t array,
 * which the CPU's store buffer coalesces efficiently.
 *
 * Only the world model gets the SoA — entity bmodels are small and
 * don't pay the same penalty. R_BspCache_Init() sets it up from
 * cl.worldmodel->surfaces on each R_NewMap.
 * ===================================================================== */
static uint32_t   *bsp_surf_visframe;
static uint8_t    *bsp_surf_flags;
static msurface_t *bsp_surf_base;
static int         bsp_surf_count;

static inline unsigned bsp_surf_idx(const msurface_t *s) {
    return (unsigned)(s - bsp_surf_base);
}

void R_BspCache_Init(void)
{
    model_t *world = cl.worldmodel;
    if (!world || !world->surfaces) {
        bsp_surf_base  = NULL;
        bsp_surf_count = 0;
        return;
    }
    bsp_surf_base  = world->surfaces;
    bsp_surf_count = world->numsurfaces;
    bsp_surf_visframe = Hunk_AllocName(
        bsp_surf_count * sizeof(uint32_t), "bspvis");
    bsp_surf_flags = Hunk_AllocName(
        bsp_surf_count * sizeof(uint8_t), "bspflg");
    for (int i = 0; i < bsp_surf_count; i++) {
        bsp_surf_visframe[i] = 0;
        bsp_surf_flags[i]    = (uint8_t)world->surfaces[i].flags;
    }
}
...
================
*/
PQ_HOT void R_RecursiveClipBPoly (bedge_t *pedges, mnode_t *pnode, msurface_t *psurf)
{
...
// if a leaf node, draw stuff
	if (node->contents < 0)
	{
		...
		if (c)
		{
			/* Dense SoA write — line-coalesced vs scattered pointer chase. */
			uint32_t *vf = bsp_surf_visframe;
			msurface_t *base = bsp_surf_base;
			uint32_t frame = r_framecount;
			do
			{
				vf[(unsigned)(*mark - base)] = frame;
				mark++;
			} while (--c);
		}
	...
	// draw stuff
		...
		if (c)
		{
			 ...
			 * 64-byte msurface_t into D-cache. R_RenderFace still
			 * needs the struct, but only when the predicate passes. */
			const uint32_t *vf = bsp_surf_visframe;
			const uint8_t  *fl = bsp_surf_flags;
			const msurface_t *base = bsp_surf_base;
			const uint32_t frame = r_framecount;
			if (dot < -BACKFACE_EPSILON)
			{
				do
				{
					unsigned idx = (unsigned)(surf - base);
					if ((fl[idx] & SURF_PLANEBACK) &&
						(vf[idx] == frame))
					{
						R_RenderFace (surf, clipflags, dot);
					}
