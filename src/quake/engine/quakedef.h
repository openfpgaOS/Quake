...
/* Hot-code placement on openfpgaOS.
 *
 * Both `PQ_FASTTEXT = section(".app_fasttext.pq")` (BRAM placement) and
 * `PQ_HOT = __attribute__((hot))` (`.text.hot.*` clustering) regressed
 * frame time on real hardware (-35% and -45 ms respectively).  The
 * fabric's I-cache is small enough that rearranging code layout —
 * even just clustering hot functions — disrupts the natural callee
 * adjacency the linker produces from declaration order.  Both macros
