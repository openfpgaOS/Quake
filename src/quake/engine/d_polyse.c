 ...
 *   2 = hardware triangle rasterizer (fastest, requires FULL GPU).
 *   1 = CPU edge-walker with GPU affine span emit.
 *   0 = pure CPU scanline (slowest, portable).
 *
 * With the full GPU now online we use mode 2 — the HW triangle
 * rasterizer handles edge walking, scanline generation, texture
 * sampling with COLORMAP, Gouraud-interpolated per-pixel light, and
 * DEPTH_TEST/WRITE with GEQUAL semantics in a single command per
 * triangle. The CPU just projects vertices and fills an
 * of_emit_vertex_t[3] per triangle. */
...
void D_DrawNonSubdiv (void)
{
	...
	/* Skin flush — once per skin change. GPU reads via AXI (cache-
	 * incoherent), so we write back any dirty D-cache lines over the
	 * skin bytes before the HW rasterizer reads them. */
	{
		static byte *last_skin_flushed = 0;
		if (r_affinetridesc.pskin != last_skin_flushed) {
			last_skin_flushed = r_affinetridesc.pskin;
			of_emit_cache_clean(r_affinetridesc.pskin,
	...
	for (i = 0; i < lnumtriangles; i++, ptri++) {
		...
		/* Mode 1/0 — CPU edge walker feeds either GPU affine spans or
		 * pure CPU scanlines (guarded below in D_PolysetDrawSpans8). */
...
void D_PolysetUpdateTables (void)
{
		...
		r_affinetridesc.pskin != skinstart)
	{
		...
		for (i=0 ; i<MAX_LBM_HEIGHT ; i++, s+=skinwidth)
			skintable[i] = s;
		/* GPU skin cache flush lives in D_DrawNonSubdiv (per-skin-change,
		 * gated on `last_skin_flushed`). The previous duplicate flush
		 * here added a syscall on every model-table update without
		 * changing GPU coherency. Removed. */
	}
}
